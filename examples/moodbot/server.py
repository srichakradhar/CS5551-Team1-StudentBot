...
class Server:
    ...
    async def respond(self, request, sender_id):
        request.setHeader('Content-Type', 'application/json')
        request.setHeader('Access-Control-Allow-Origin', '*')
        request_params = request_parameters(request)
        if 'query' in request_params:
        ...
        try:
            ...
            response = await self.agent.handle_text(message, output_channel=out, sender_id=sender_id)
            request.setResponseCode(200)
            return json.dumps(response)
        except Exception as e:
            request.setResponseCode(500)
...
if __name__ == "__main__":
    server = Server("models/", RasaNLUInterpreter("models/nlu"))
